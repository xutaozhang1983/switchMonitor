<template>
  <!-- 导入表 -->
  <el-dialog title="导入表" v-model="genStore.showImportDialog" width="800px" top="5vh" append-to-body>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button @click="genStore.showImportDialog = false">取 消</el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
<script setup lang="ts">
  ...
  function handleImportTable() {
    ...
    importTable({ tables: tableNames }).then((res: any) => {
      ...
      if (res.code === 200) {
        genStore.showImportDialog = false
        emit('ok')
      }
    })
  }
